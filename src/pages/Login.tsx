...
function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('owncorp_user', JSON.stringify({
        id: user.id,
    ...
    <div className="min-h-screen bg-light flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-lg text-medium">
          Access your Own Corp business dashboard
        </p>
      </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <input
                  ...
                  name="remember-me"
                  type="checkbox"
                  className="h-5 w-5 text-accent focus:ring-accent border-accent/30 rounded-lg"
                />
                <label htmlFor="remember-me" className="ml-3 block text-sm text-medium font-medium">
                  Remember me
                </label>
              </div>

              <div className="text-sm">
                <a href="#" className="font-semibold text-dark hover:text-accent transition-colors duration-200">
                  Forgot password?
                </a>
              </div>
            </div>
